...
func TestCreateService(t *testing.T) {
	...
	mockSvc.EXPECT().CreateService(gomock.Any()).Return(resp, nil)
	...
	testResp, err := e.CreateService(s, "test-lb", "taskdef")

	assert.NoError(t, err)

	assert.Equal(t, "test-service", testResp)
}
...
func TestCreateServiceError(t *testing.T) {
	...
	// Create a new mock controller and add the response to the controller
	mockSvc := mocks.NewMockECSAPI(ctrl)
	mockSvc.EXPECT().CreateService(gomock.Any()).Return(nil, errors.New("I got a error"))
	// Create a mock implementation
	e := EcsImplementation{
		Svc: mockSvc,
	}

	_, err := e.CreateService(&parser.Definition{}, "", "¨")
...
func TestCreateTaskDefinition(t *testing.T) {
	...
	testResp, err := e.CreateTaskDefinition(s)
